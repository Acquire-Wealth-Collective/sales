...
const useDocConfigStore = create((set) => ({
  selectionsByEngagement: {},
  contactsByEngagement: {},
  dueDateByEngagement: {},
  notesByEngagement: {},
  setSelections: (engId, items) => set((s) => ({ selectionsByEngagement: { ...s.selectionsByEngagement, [engId]: items } })),
  seedIfEmpty: (engId, items) => set(
    (s) => engId in s.selectionsByEngagement ? s : { selectionsByEngagement: { ...s.selectionsByEngagement, [engId]: items } }
  ),
  toggleItem: (engId, item) => set((s) => {
    const cur = s.selectionsByEngagement[engId] ?? [];
    const next = cur.includes(item) ? cur.filter((x) => x !== item) : [...cur, item];
    return { selectionsByEngagement: { ...s.selectionsByEngagement, [engId]: next } };
  }),
  setCategoryAll: (engId, categoryId, on) => set((s) => {
  ...
  toggleContact: (engId, contactId) => set((s) => {
  ...
  setDueDate: (engId, d) => set((s) => ({ dueDateByEngagement: { ...s.dueDateByEngagement, [engId]: d } })),
  setNotes: (engId, n) => set((s) => ({ notesByEngagement: { ...s.notesByEngagement, [engId]: n } })),
  reset: (engId) => set((s) => {
...
const useDocContactsStore = create((set, get) => ({
...
export {
  DOC_CATEGORIES as D,
  useDocContactsStore as a,
  useDocConfigStore as u
};
